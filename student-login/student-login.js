...
if (signupForm) {
    signupForm.addEventListener('submit', async (e) => {
        ...
        const formData = {
            ...
            password: password,
            role: 'student',
            identifier: '' // Students might not have one in this form
        };
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showToast(result.error || result.message || "Login failed", "error");
            }
        } catch (error) {
    ...
    fpResetBtn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/forgot-password/reset`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email, otp, newPassword, role: 'student' })
            });
            const data = await res.json();
